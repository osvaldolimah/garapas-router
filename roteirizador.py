...
# --- 2. DESIGN SYSTEM (TRAVADO E CENTRALIZADO) ---
...
st.markdown("""
    ...
    * { box-sizing: border-box !important; }
    ...
    /* CARDS */
    .delivery-card { border-radius: 8px; padding: 6px; background-color: white; border-left: 4px solid #FF4B4B; margin: 6px 0; }
...
if st.session_state['df_final'] is None:
    ...
    if uploaded_file and st.button("🚀 Otimizar Rota", use_container_width=True):
        with st.spinner("Traçando melhor rota..."):
...
if st.session_state['df_final'] is not None:
    ...
    st_folium(m, width=None, height=320, use_container_width=True, key="mapa_fixo")
    ...
    # C. BOTÃO LIMPAR COM SPINNER
    if st.button("🗑️ LIMPAR FEITAS", use_container_width=True):
        if restantes:
            with st.spinner("Atualizando mapa e rota..."):
                ...
                # Recalcula a rota no servidor antes de atualizar a página
